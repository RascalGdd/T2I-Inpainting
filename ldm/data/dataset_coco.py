...
class dataset_cod_mask_color():
    ...
    def __getitem__(self, idx):
        ...
        color = cv2.imread(os.path.join(self.root_path_mask, name))  # [:,:,0]
        color = cv2.resize(mask, (512, 512))
        color = img2tensor(mask, bgr2rgb=True, float32=True) / 255.  # [0].unsqueeze(0)#/255.
